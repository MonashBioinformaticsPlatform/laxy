...
class FileViewTest(TestCase):
    ...
    def test_job_file_download_content(self):
        ...
        url = reverse('laxy_backend:job_file',
                      args=[job.id, f'{self.file_on_disk.full_path}'])
        response = self.user_client.get(f'{url}?download')
    ...
    def test_job_file_put(self):
        ...
        self.assertEqual(response.status_code, 200)
        job = Job.objects.get(id=job.id)
        self.assertEqual(laxified_url,
                         job.output_files.get_files().first().location)
...
class JobViewTest(TestCase):
    ...
    def test_job_files_from_csv_with_location(self):
        ...
        self.assertListEqual(job.input_files.get_files()[0].type_tags,
                             ['fastq', 'ena'])
