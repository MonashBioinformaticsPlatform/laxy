...
from laxy_backend.views import (
    ...
    ENAQueryView,
    ENAFastqUrlQueryView,
    ENASpeciesLookupView,
    JobListView,
    EventLogCreate,
    EventLogListView,
    JobEventLogCreate,
    ...
    FileSetTarDownload,
    JobClone,
    ReferenceGenomesView,
)

from laxy_backend.view_auth import (
    Login,
    Logout,
    PublicSocialSessionAuthView,
    CsrfCookieView,
    check_drf_token,
    UserProfileView,
)
...
rest_social_auth_urls = [
    ...
    re_path(
        r"^social/session/(?:(?P<provider>[a-zA-Z0-9_-]+)/?)?$",
...
api_urls = [
    ...
    re_path(r"genomes/$", ReferenceGenomesView.as_view(), name="genomes"),
    ...
    re_path(r"auth/login/$", Login.as_view(), name="api_login"),
    re_path(r"auth/logout/$", Logout.as_view(), name="api_logout"),
    ...
    re_path(
        r"^auth/social/oauth2/",
        include(
            ("rest_framework_social_oauth2.urls", app_name),
    ...
    re_path(
        r"job/(?P<uuid>[a-zA-Z0-9\-_]+)/event/$",
        JobEventLogCreate.as_view(),
        name="create_job_eventlog",
    ),
    re_path(
        r"job/(?P<job_id>[a-zA-Z0-9\-_]+)/accesstoken/$",
    ...
    re_path(
        r"job/(?P<job_id>[a-zA-Z0-9\-_]+).tar.gz$",
        JobDirectTarDownload.as_view(),
        name="job_tarball_download",
    ),
    re_path(
    ...
    re_path(r"job/(?P<uuid>[a-zA-Z0-9\-_]+)/$", JobView.as_view(), name="job"),
    re_path(r"file/$", FileCreate.as_view(), name="create_file"),
    re_path(
        r"file/(?P<uuid>[a-zA-Z0-9\-_]+)/content/(?P<filename>.*)$",
    ...
    re_path(
        r"samplecart/(?P<uuid>[a-zA-Z0-9\-_]+)/$",
        SampleCartView.as_view(),
        name="samplecart",
    ),
    re_path(r"samplecart/$", SampleCartCreate.as_view(), name="create_samplecart"),
    re_path(
        r"pipelinerun/(?P<uuid>[a-zA-Z0-9\-_]+)/$",
        PipelineRunView.as_view(),
        name="pipelinerun",
    ),
    re_path(r"pipelinerun/$", PipelineRunCreate.as_view(), name="create_pipelinerun"),
    re_path(
    ...
    re_path(
        r"compute-resource/$", ComputeResourceCreate.as_view(), name="compute_resource"
    ),
    re_path(
        r"pipeline/(?P<uuid>[a-zA-Z0-9\-_]+)/$", PipelineView.as_view(), name="pipeline"
    ),
    re_path(r"pipelines/$", PipelineListView.as_view(), name="pipelines"),
    re_path(r"ena/$", ENAQueryView.as_view(), name="ena_query"),
    re_path(
        r"ena/fastqs/$", ENAFastqUrlQueryView.as_view(), name="ena_fastq_url_query"
    ),
    re_path(
        r"ena/species/(?P<accession>[a-zA-Z0-9\-_]+)$",
        ENASpeciesLookupView.as_view(),
        name="ena_species_from_sample_query",
    ),
    re_path(r"eventlogs/$", EventLogListView.as_view(), name="list_eventlogs"),
    re_path(r"eventlog/$", EventLogCreate.as_view(), name="create_eventlog"),
    re_path(r"remote-browse/$", RemoteBrowseView.as_view(), name="remote-browse"),
    re_path(r"accesstoken/$", AccessTokenCreate.as_view(), name="create_accesstoken"),
    re_path(
        r"accesstoken/(?P<uuid>[a-zA-Z0-9\-_]+)/$",
        AccessTokenView.as_view(),
        name="accesstoken",
    ),
    re_path(r"accesstokens/$", AccessTokenListView.as_view(), name="list_accesstokens"),
    re_path(
        r"action/send-to/degust/(?P<file_id>[a-zA-Z0-9\-_]+)/$",
