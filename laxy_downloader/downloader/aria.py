...
def download_urls(urls: List[str], cache_path=None, proxy=None, concurrent_downloads=8):
    aria = get_daemon()
    default_options = {'continue': 'true',
...
def downloads_finished(download_ids):
    ...
    done = all([dl.get('status', None) == 'complete' or
                dl.get('status', None) == 'error' or
                dl.get('status', None) == 'removed'
                for dl in statuses])
    return done
