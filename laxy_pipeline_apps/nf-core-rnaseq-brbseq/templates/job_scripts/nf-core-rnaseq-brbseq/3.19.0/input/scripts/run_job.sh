...
function normalize_annotations() {
    ...
    [[ -n "${NFCORE_EXTRA_ATTRIBUTES}" ]] &&         ANNOTATION_FLAGS+=" --gtf_extra_attributes ${NFCORE_EXTRA_ATTRIBUTES}"
...
function get_settings_from_pipeline_config() {
    ...
    local _debug_mode=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].debug_mode // "false"' "${PIPELINE_CONFIG}")
    export USER_DEBUG_MODE="no"
    if [[ "${_debug_mode}" == "true" ]]; then
        export USER_DEBUG_MODE="yes"
    fi

    local _has_umi=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].has_umi // "false"' "${PIPELINE_CONFIG}")
    export UMI_FLAGS=""
    if [[ "${_has_umi}" == "true" ]]; then
        export UMI_FLAGS=" --with_umi --skip_umi_extract --umitools_umi_separator : "
    fi

    local _skip_trimming=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].skip_trimming // "false"' "${PIPELINE_CONFIG}")
    export EXTRA_FLAGS=""
    if [[ "${_skip_trimming}" == "true" ]]; then
        export EXTRA_FLAGS="${EXTRA_FLAGS} --skip_trimming "
    fi

    local -i _min_mapped_reads=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].min_mapped_reads // "5"' "${PIPELINE_CONFIG}")
    ...
    local -i _min_trimmed_reads=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].min_trimmed_reads // "10000"' "${PIPELINE_CONFIG}")
    export MIN_TRIMMED_READS_ARG=" --min_trimmed_reads ${_min_trimmed_reads} "
    
    local _trimmer=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].trimmer // ""' "${PIPELINE_CONFIG}")
    TRIMMER_ARGS=()
...
function cleanup_nextflow_intermediates_keep_work_logs() {
    local _save_genome_index=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].save_genome_index // "false"' "${PIPELINE_CONFIG}")
    if [[ ${_save_genome_index} != 'true' ]]; then
        rm -rf "${JOB_PATH}/output/results/genome/index"
    fi

    local _save_reference_genome=$(jq --raw-output '.params["'${PIPELINE_NAME}'"].save_reference_genome // "false"' "${PIPELINE_CONFIG}")
