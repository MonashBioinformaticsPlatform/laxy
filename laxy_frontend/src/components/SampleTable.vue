    ...
    <md-table-header>
      <md-table-row>
        <md-table-head
          ...
          :key="field"
        >{{ field.split('metadata.').pop() | deunderscore }}</md-table-head>
      </md-table-row>
    </md-table-header>
    <md-table-body>
      <md-table-row
        v-for="sample in samples.items"
        :key="sha1(sample)"
        :md-item="sample"
        ...
        <md-table-cell v-for="field in fields" :key="field">
          <span v-if="editableFields.includes(field)">
            <md-input-container v-if="field.startsWith('metadata.')">
              <md-input
                v-model="sample.metadata[field.split('metadata.').pop()]"
                :placeholder="field.split('metadata.').pop()"
              ></md-input>
            </md-input-container>
            <md-input-container v-else>
              <md-input v-model="sample[field]" :placeholder="field"></md-input>
            </md-input-container>
          </span>
          <span
            v-else-if="field.endsWith('read_count')"
          >{{ deep_path(sample, field) | numeral_format('0 a') }}</span>
          <span v-else-if="field === 'R1' || field === 'R2'">
            <span
              v-for="file in filter(sample.files, (f) => get(f, field, false))"
              :key="file[field].location"
            >
              {{ file[field].name }}
              <md-tooltip>{{ file[field].location }}</md-tooltip>
            </span>
          </span>
          <span v-else>{{ deep_path(sample, field) }}</span>
        </md-table-cell>
      </md-table-row>
    </md-table-body>
  </md-table>
