  ...
  <md-card :style="cssStripe(getThemedStatusColor(job.status), 'left', 4)">
    <md-card-header>
      <md-card-header-text>
        ...
        <div
          v-if="job.status != 'running' && job.completed_time"
          class="md-subhead"
        >It took about {{ job.created_time | moment("from", job.completed_time, true) }}</div>
        <div
          v-if="job.status == 'running'"
          class="md-subhead"
        >for {{ job.created_time | moment("from", 'now', true) }}</div>
      </md-card-header-text>
      ...
      <md-card-media>
        ...
        <img
          v-if="job.status == 'running'"
          src="assets/outline-directions_run-24px.svg"
          alt="running"
        />
        <img
          v-if="job.status == 'complete'"
          src="assets/outline-check_circle_outline-24px.svg"
          alt="complete"
        />
        <img
          v-if="job.status == 'cancelled'"
          src="assets/outline-cancel_presentation-24px.svg"
          alt="cancelled"
        />
        <img v-if="job.status == 'failed'" src="assets/outline-error_outline-24px.svg" alt="failed" />
      </md-card-media>
    </md-card-header>
    ...
    <md-card-actions>
      ...
      -->
      <md-button v-if="job.status === 'running'" @click="askCancelJob(job.id)">
