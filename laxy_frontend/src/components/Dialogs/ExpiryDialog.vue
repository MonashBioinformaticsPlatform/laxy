<template>
  <md-dialog :md-open-from="openFrom" :md-close-to="closeTo" id="thisDialog" ref="thisDialog">
    ...
    <md-dialog-content>
      Large files associated with this job will be deleted
      <template v-if="job">
        on
        <strong>{{ job.expiry_time | moment('DD-MMM-YYYY (HH:mm UTCZ)') }}</strong>
      </template>.
      <br />If you wish to preserve these files, please archive them to another location.
      <br />Small files such as reports and logs will be maintained.
    </md-dialog-content>
